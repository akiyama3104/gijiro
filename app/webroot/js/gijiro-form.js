$(function () {
    ...
    var original = $(".content_block\\[" + frm_cnt + "\\]");
    var originCnt = frm_cnt;

    $(document).on("click", ".add-heading",{i:1}, function(event) {
        var last_heading = $(".content-block").find(":last"),
            heading_idx = String($(".heading").length),
            ...
            format = "<li id=\"heading_" + heading_idx + " \" class=\"heading\">見出し：<div class=\"input text\">" +
                ...
                "<span><button class=\"add-btn\" type=\"button\" id=\"heading-" + heading_idx + "add\">+</button></span>\n" +
                "<span><button class=\"remove-btn\" type=\"button\">-</button></span>\n" +
                "<ul>\n" +
                "<li>\n" +
    ...
    $(".form-extension").keydown(function(e){
        if(event.shiftKey){
            if(e.keyCode === 13){
